...
def extract_text_from_bytes(content_bytes: bytes, filename: str = "resume.pdf") -> str:
    """Extracts raw text directly from in-memory bytes of PDF, DOCX, or TXT."""
    suffix = Path(filename).suffix.lower()
    text = ""

    if suffix == ".pdf":
        # 1. Primary: Pure-Python pypdf (100% serverless/Lambda compatible, no C++ dependencies)
        try:
            import pypdf
            ...
            logger.warning(f"pypdf in-memory extraction notice: {e_pypdf}")

        # 2. Secondary fallback: PyMuPDF (fitz) if pypdf extracted nothing
        if len(text.strip()) < 15:
            try:
                ...
                logger.warning(f"fitz in-memory extraction notice: {e_fitz}")
        ...
        try:
            ...
            logger.warning(f"docx in-memory extraction notice: {e_docx}")

    elif suffix in [".txt", ".md"]:
        text = content_bytes.decode("utf-8", errors="ignore")
    else:
        # Default text decode attempt
        text = content_bytes.decode("utf-8", errors="ignore")

    clean_text = re.sub(r"\n\s*\n+", "\n\n", text).strip()
    if not clean_text:
        raise ValueError(f"No extractable text found in '{filename}'. Ensure document contains selectable text.")
    return clean_text


def extract_text_from_file(file_path: Union[str, Path]) -> str:
    """Extracts raw text from PDF, DOCX, or TXT file on disk."""
    path = Path(file_path)
    if not path.exists():
    ...
    return extract_text_from_bytes(content_bytes, filename=path.name)
...
def parse_resume(
    file_path_or_bytes_or_text: Union[str, Path, bytes],
    is_raw_text: bool = False,
    filename: str = "resume.pdf"
) -> Dict[str, Any]:
    """
    LangChain tool-compatible function: parses a resume into structured JSON.
    Takes either a file path, raw in-memory bytes, or raw resume text.
    """
    try:
        if isinstance(file_path_or_bytes_or_text, bytes):
            raw_text = extract_text_from_bytes(file_path_or_bytes_or_text, filename=filename)
            source_name = filename
        elif is_raw_text:
            raw_text = str(file_path_or_bytes_or_text)
            source_name = "Direct Input"
        elif isinstance(file_path_or_bytes_or_text, (str, Path)) and Path(str(file_path_or_bytes_or_text)).exists():
            raw_text = extract_text_from_file(file_path_or_bytes_or_text)
            source_name = Path(file_path_or_bytes_or_text).name
        else:
            raw_text = str(file_path_or_bytes_or_text)
            source_name = "Direct Input"
